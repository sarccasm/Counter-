function getCounterpicks(heroName) {
    ...
    if (counterpick.length > 0) {
        console.log("Контрпіки для: ", heroName + ":");
        for (let i = 0; i < counterpick.length; i++) {
            console.log(counterpick[i]);
        }
    } else {
        console.log("Контрпіки для: ", heroName, "нема.");
    }
}
...
rl.question('Введіть ім\'я героя: ', (heroName) => {
    getCounterpicks(heroName);
    rl.close();
});
